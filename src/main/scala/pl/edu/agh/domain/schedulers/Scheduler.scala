...
import pl.edu.agh.domain.{DataCenter, Application, Machine, Parameters}
...
trait Scheduler {
  ...
  def canScheduleApplicationConsideringParamsOnly(application: Application, machine: Machine): Boolean = {
    ...
    calculated.cpu >= 0 && calculated.ram >= 0 && calculated.disk >= 0 && calculated.dpdkVirtualNICs >= 0
  }
